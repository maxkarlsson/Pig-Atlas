...
if(file.exists("data/processed/pig_umap.Rdata")) {
  load("data/processed/pig_umap.Rdata")
} else {
  ...
  sample_umap <- 
    ...
    uwot::umap() %>%
    as_tibble() %>% 
    mutate(sample_ID = sample_umap_data$sample_ID) %>% 
  ...
  tissue_umap <- 
    ...
    uwot::umap() %>%
    as_tibble() %>% 
    mutate(tissue_name = tissue_umap_data$tissue_name) %>% 
    select(tissue_name, V1, V2)
  
  save(list = c("sample_umap",
                "tissue_umap"),
       file = "data/processed/pig_umap.Rdata")
}
...
plot_pca$stats %>% 
  mutate(R2 = R2cum - c(0, R2cum[-length(R2cum)])) %>% 
  ggplot(aes(R2, R2cum)) +
  geom_line()+
  geom_point()
...
plot_umap2 <- 
  plot_pca$scores[,1:20] %>%
  # uwot::umap(n_components = 2, metric = "cosine") %>%
  umapr::umap(metric = "haversine") 
  as_tibble() %>% 
  mutate(sample_ID = sample_umap_data$sample_ID) 
...
plot_umap3 <- 
  plot_pca$scores[,1:20] %>%
  uwot::umap(n_components = 2, metric = "manhattan") %>%
  as_tibble() %>% 
  mutate(sample_ID = sample_umap_data$sample_ID) 
...
plot_umap4 <- 
  plot_pca$scores[,1:20] %>%
  uwot::umap(n_components = 2, metric = "hamming") %>%
  as_tibble() %>% 
  mutate(sample_ID = sample_umap_data$sample_ID) 
...
plot_umap1 %$%
  scatterplot3d::scatterplot3d(sin(V1) * cos(V2), sin(V1)*sin(V2), cos(V1))

plot_umap2 %$%
  scatterplot3d::scatterplot3d(V1, V2, V1)
plot_umap2 %$%
  car::scatter3d(sin(V1) * cos(V2), sin(V1)*sin(V2), cos(V1),
...
pig_gene_umap_density <- 
  pig_gene_umap %$% 
  MASS::kde2d(V1, V2, n = 500, h = 0.5) %>% 
  {.$z} %>%
...
plot_x <- unique(plot_data$x)

plot_y <- unique(plot_data$y)
...
persp3d(x = plot_x, 
        y= plot_y, 
        z = plot_z,
        zlim = c(0.005, max(plot_z)),
        color = plot_col,
        aspect = c(1, 1, 0.1),
        
        front = "lines",
        back = "lines")
# rgl::mesh(x, y, density,alpha = 0.5,
#             type = "",
#             # size = 5,
#             col = color, aspect = c(1,1,0.1))
# 
# car::scatter3d(sin(x) * cos(y), sin(x)*sin(y), cos(x),
#                bg.col = "black",
#                surface.col = 1:10,
#                surface=FALSE)
# 
...
# intraconsens_gene_enrich_overlap %>%
#   write_csv("data/processed/intraconsensus results.csv")
...
intraconsens_gene_enrich_overlap %>%
  select(1:9) %>%
  write_csv("data/processed/intraconsensus results small.csv")

intraconsens_gene_enrich_overlap %>%
  group_by(enssscg_id, gene_name) %>% 
  summarise(consensus_tissue_name = paste(sort(unique(consensus_tissue_name)),
                                          collapse = ";")) %>%
  write_csv("data/processed/intraconsensus results summarised.csv")

# intraconsens_gene_enrich_overlap_all %>%
#   arrange(consensus_tissue_name, enssscg_id) %>%
#   write_csv("data/processed/intraconsensus results including elevated.csv")
...
# load(file = "data/processed/pig_intraconsensus_anova.Rdata")
# intraconsens_anova_res %>%
#   select(1,2,5,6,8,9,13,14) %>%
#   select(1,2,5,6,7,3,4,8) %>%
#   filter(ratio > 4 | ratio < 1/4) %>%
#   # mutate(tissue1 = unclass(factor(tissue1)),
#   #        tissue2 = unclass(factor(tissue2)),
#   #        ratio = round(ratio,5),
#   #        t = round(t,5),
#   #        pval = round(pval,5),
#   #        adj_p_global = round(adj_p_global,5)) %>%
#   arrange(adj_p_global) %>% 
#   write_csv("data/processed/intraconsensus model results.csv")
######
...
plot_data1 <- 
  read_delim("data/meta/fig1A data.txt", delim = "\t") %>%
  filter(organ_name != "Pancreas") %>%
...
plot_data2 %>%
  ...
  ggplot(aes(column, plot_order, label = label, fill = label)) +
  geom_segment(data = plot_data3,
               aes(x = 1, 
                   xend = 2,
                y = left_pos, yend = plot_order,
                color = tissue_name),
               size = 2,
               # alpha = 0.5,
               inherit.aes = F,
  ...
  #           )
  geom_text(data = . %>% 
              filter(column == "consensus_tissue_name"),
            show.legend = F,
            # label.size = 0,
            hjust = 1,
...
###UMAP
```{r Figure1_UMAP, echo=FALSE, message=FALSE, warning=FALSE}
...
g <- 
  ...
  coord_fixed()
g
ggsave(savepath("Figure1 UMAP.pdf"), plot = g, width = 5, height = 4, useDingbats = F)
```
...
plot_data <-
  pig_gene_classification %>% 
  select(enssscg_id, specificity_category, distribution_category) %>% 
  ...
  mutate(perc = 100 * n / sum(n),
         label = paste0(n, "\n", round(perc, 1), "%")) %>% 
  ungroup() %>% 
  mutate(class = factor(str_to_sentence(class), unique(c(spec_category_levels, dist_category_levels), fromLast = T)),
         class_type = factor(str_to_sentence(gsub("_category", "", class_type)),
...
ggsave(savepath("Class pies.pdf"), 
       width = 5, height = 4)

```

### Classification ezamples 

```{r}
gene_mapping %>% 
  filter(display_name %in% c("MOG",
                             "MYL2",
                             "A2M",
                             "RPS16")) %>% 
  inner_join(pig_gene_classification) %>% 
  inner_join(pig_atlas_consensus) %>% 
  inner_join(tissue_mapping %>% 
               select(consensus_tissue_name, organ_name) %>% 
               distinct(),
  ...
  ggplot(aes(tissue, nx, fill = tissue)) +
  geom_col(show.legend = F) +
  facet_wrap(~ display_name + specificity_category,
             scales = "free", ncol = 1, strip.position = "left") +
  scale_fill_manual(values = tissue_colors_palette_full) +
  ...
  theme(axis.text = element_blank(),
        ...
        panel.spacing = unit(0, "mm"))
ggsave(savepath("Classification examples.pdf"), 
       width = 4, height = 4)
...
ggsave(savepath("Figure2 barplot short.pdf"), width = 5, height = 5)
```
...
plot_genes <- c("MOG", "ASGR1", "CRISP2", "TNNT1", "DSC1")
...
plot_data <-
  ...
  inner_join(gene_mapping %>% 
               filter(display_name %in% plot_genes)) %>%
  left_join(tissue_mapping %>% 
              select(consensus_tissue_name, organ_name) %>% 
              distinct(),
            by = c("tissue" = "consensus_tissue_name")) %>%
  arrange(organ_name) %>%
  mutate(display_name = factor(display_name, plot_genes),
...
pig_gene_classification %>%
  classification_network_plot_3(gene_col = "enssscg_id",
                                spec_col = "specificity_category",
                                enriched_col = "enhanced_tissues",
                                spec_filter = c("Tissue enriched", 
                                                "Group enriched"),
                                ...
                                node_filter_show_cat = "Tissue enriched",
                                node_filter_min = 3,
                                node_filter_n_show = 10, 
                                frac_show = 0.1,
                                scale_factor = 1)

ggsave(savepath("Network pig enrichment whole group 2.pdf"), width = 16, height = 12, useDingbats = F)
...
color_data <- 
  ...
  arrange(merged_cluster, -mean_exp) %>%
  ungroup() 

cluster_contribution_color <- 
  color_data %>%
  filter(mean_exp > 0.02) %>%
  # filter(merged_cluster == 1) %>%
  left_join(tissue_colors_palette_full %>% 
              enframe("tissue", "color")) %>%
  bind_cols(col2rgb(.$color) %>% 
              t() %>% 
              as_tibble()) %>%
  group_by(merged_cluster) %>%
  mutate(mean_exp = mean_exp ^ 2,
         mean_exp = mean_exp / sum(mean_exp),
         red = red * mean_exp,
         green = green * mean_exp,
         blue = blue * mean_exp) %>%
  group_by(merged_cluster) %>% 
  ...
  set_names(color, merged_cluster)
####
...
pig_gene_umap %>%
  ...
  ggrastr::geom_point_rast(size = 0.2,
                           shape = 16,
                           stroke = 0, raster.dpi = 600) +
  stripped_theme_facet +
  coord_fixed() +
  labs(x = "UMAP1", 
       y = "UMAP2")
ggsave(savepath("Gene UMAP points.pdf"), width = plot_width, height = plot_height)
...
plot_ref <- 
  pig_gene_umap %>%
  ggplot(aes(V1, V2)) +
  geom_hex(bins = plot_bins)

plot_specs <- hex_binwidth(plot_ref, plot_bins)
...
plot <- 
  ...
  geom_hex(aes(fill = stat(log10(count))),
           bins = plot_bins,
           binwidth = plot_specs$binwidth * c(1, plot_specs$ratio),
           show.legend = T) +
...
ggsave(savepath("Gene UMAP hex density.pdf"), width = plot_width, height = plot_height)

pig_gene_umap %>%
  ggplot(aes(V1, V2)) +
  geom_density2d_filled(bins = 300,
                        h = 0.4,
                        show.legend = F) +
  # scale_fill_viridis_c() +
  # scale_color_viridis_c() +
...
ggsave(savepath("Gene UMAP point.png"), plot = plot, width = plot_width, height = plot_height)
####
library(ggalt)
...
umap_pal <- 
  colorRampPalette(ggthemes::tableau_color_pal(palette = "Hue Circle", 
                                             type = c("regular"), 
                                             direction = 1)(19))(n_distinct(pig_gene_umap$merged_cluster))
...
plot <- 
  ...
  geom_text(data = . %>% 
            ...
            show.legend = F) +
  stripped_theme +
  scale_fill_manual(values = umap_pal) +
  scale_color_manual(values = umap_pal) +
  coord_fixed()
...
#             show.legend = F) +
#   stripped_theme +
#   scale_fill_manual(values = cluster_contribution_color) +
#   scale_color_manual(values = cluster_contribution_color) 
# 
# ggsave(savepath("Gene UMAP clusters tissue colors.pdf"), plot = plot, width = plot_width, height = plot_height)
...
plot <- 
  ...
  plot2 +
  plot_layout(ncol = 1)
ggsave(savepath("Gene UMAP examples.pdf"), plot = plot, width = (plot_width * 2)/2, height = plot_height * 2)
...
plot <- 
  ...
  geom_hex(data = plot_data2 %>%
           ...
           binwidth = plot_specs$binwidth * c(1, plot_specs$ratio),
           show.legend = F) +
  facet_wrap(~ enhanced_tissues, ncol = 5) +
  theme_void() +
  theme(strip.text = element_text(size = 8)) +
  coord_fixed()

# plot
ggsave(savepath("Gene UMAP all tissues.pdf"), plot = plot, 
       width = 174, 
       height = 247, 
       units = "mm")
...
plot_data2_zoom <-
  plot_data2 %>% 
  filter(V1 > plot_data_zoom_lims$xmin,
         V1 < plot_data_zoom_lims$xmax,
         V2 > plot_data_zoom_lims$ymin,
         V2 < plot_data_zoom_lims$ymax) %>%
  filter(enhanced_tissues %in% 
           c("lung", 
             "testis"))
...
plot_specs_zoom <- hex_binwidth(plot_ref_zoom, plot_bins)
...
#   geom_hex(fill = "lightgray",
#            bins = 30) +
#            # binwidth = plot_specs_zoom$binwidth * c(1, plot_specs_zoom$ratio)) +
#   geom_hex(data = plot_data2 %>% 
#              filter(V1 > -2,
#                     V1 < 0,
#                     V2 > 1.2,
...
plot_data_zoom %>%
  mutate(in_cluster = merged_cluster == 23) %>%
  select(-enhanced_tissues) %>%
  
  
  ggplot(aes(V1, V2, color = in_cluster)) +
  geom_point(size = 0.5,
             show.legend = F) +
  # binwidth = plot_specs_zoom$binwidth * c(1, plot_specs_zoom$ratio)) +
  geom_point(data = plot_data2_zoom,
             size = 0.5,
             color = "red",
             # alpha = 0.2,
             show.legend = F) +
  ...
  scale_x_continuous(limits = c(plot_data_zoom_lims$xmin, plot_data_zoom_lims$xmax)) +
  scale_y_continuous(limits = c(plot_data_zoom_lims$ymin, plot_data_zoom_lims$ymax)) +
  theme_void() +
  coord_fixed() +
  theme(panel.border = element_rect(fill = NA))

ggsave(savepath("Gene UMAP examples zoom.pdf"), 
       width = plot_width, 
       height = plot_height)
```
...
plot_data2 <- 
  ...
  mutate(label = paste0(merged_cluster, "\n", cluster_name)) #%>%
  # mutate(label = paste(strwrap(cluster_name, width = 10),
  #                      collapse = "\n")) #%>%
  # filter(!cluster_name %in% c("Low abundant uncharacterized", "Not detected"))
...
plot <- 
  ...
  geom_polygon(data = plot_data_hulls,
               ...
               show.legend = F) +
  geom_text(data = plot_data2,
            aes(mean_V1, mean_V2, label = label),
            size = 2 * 5 / 6,
            alpha = 0.6,
            show.legend = F) +
  stripped_theme +
...
plot
ggsave(savepath("Gene UMAP clusters annotated.pdf"), plot = plot, 
       width = 6.6, 
       height = 6.6)
...
plot <- 
  ...
  geom_polygon(data = plot_data_hulls,
               ...
               show.legend = F) +
  geom_text(data = plot_data2,
            aes(mean_V1, mean_V2, label = cluster_name),
            size = 2 * 5/6,
            alpha = 0.6,
            show.legend = F) +
  stripped_theme +
...
ggsave(savepath("Gene UMAP clusters tissue colors annotated.pdf"), plot = plot, 
       width = 6.6, 
       height = 6.6)
...
ggsave(savepath("Gene UMAP clusters tissue colors small numbered.pdf"), plot = plot, 
       width = 3, 
       height = 3)
```
...
rayshader::plot_gg(plot, width = 5, height = 5, scale = 30, multicore = TRUE, windowsize = c(1000, 800))
...
rayshader::plot_gg(plot, width = 5, height = 5, scale = 100, multicore = TRUE, windowsize = c(1000, 800))
  
terrain_palette <-
  (grDevices::terrain.colors(n = 20))[-c(1:2)] %>%
...
terrain_palette %>% prismatic::color()
...
plot <- 
  ...
  scale_color_gradientn(colours = terrain_palette) +
  # theme_void() +
  stripped_theme +
  theme(panel.background = element_rect(fill = "#112640"))#,
        # axis.title = element_blank(),
        # axis.text = element_blank(),
        # axis.ticks = element_blank())
plot
rayshader::plot_gg(plot, 
                   width = 5,
                   height = 5, 
                   scale = 100, 
                   multicore = TRUE, 
...
ggplot() + 
  ...
  ####
  plot_layout(widths = c(0.09848485, 1), 
              heights = c(0.2, 1))

((0.2/1.2)*6.5)/11
ggsave(savepath("UMAP hyper heatmap elevated.pdf"), width = 11, height = 6.5)  
  ...
  ####
  plot_layout(widths = c(1, 0.09848485), 
              heights = c(0.2, 1))

((0.2/1.2)*6.5)/11
ggsave(savepath("UMAP hyper heatmap elevated2.pdf"), width = 11, height = 6.5)  
...
plot_data %>% 
  ...
  distinct() %>% 
  ggplot(aes(1, merged_cluster, fill = merged_cluster)) +
  geom_tile(show.legend = F) + 
  theme_void() +
  scale_fill_manual(values = set_names(umap_pal, 1:84)) + 
  coord_fixed()
ggsave(savepath("UMAP hyper heatmap color column.pdf"), height = 6.5, width = 1)
...
plot2 <- 
  ...
  mutate(merged_cluster = factor(merged_cluster, labels(cluster_clus)),
         ...
         label = ifelse(mean_exp < 0.1, NA, label)) %>%
  ggplot(aes(mean_exp, merged_cluster, fill = plot_id)) +
  geom_col(color = "white",
           show.legend = F, 
           size = 0.3) +
...
plot3 <- 
  ...
  mutate(merged_cluster = factor(merged_cluster, labels(cluster_clus))) %>%
  left_join(UMAP_annotations %>%
              mutate(cluster_name = gsub("Housekeeping", "Hk", cluster_name) %>% 
                       gsub("Low abundant", "Low", .) %>% 
                       gsub("Uncharacterized", "Un", .))) %>% 
  arrange(merged_cluster) %>%
  mutate(cluster_label = factor(paste(merged_cluster, cluster_name),
                                unique(paste(merged_cluster, cluster_name)))) %>%
  ggplot(aes(log10(max_exp), merged_cluster)) +
  geom_violin(draw_quantiles = 0.5,
              scale = "width",
              fill = "lightgray") + 
...
pig_gene_UMAP_GO_res <-
  ...
  do({
    save_data <<- .$human_gene_name
    enrichr(save_data,
            databases = c("GO_Biological_Process_2018",
                          "GO_Cellular_Component_2018",
                          "GO_Molecular_Function_2018")) %>%
      map(.f = function(x) mutate(x, 
                                  Term = as.character(Term),
                                  Overlap = as.character(Overlap),
                                  P.value = as.numeric(P.value),
                                  Adjusted.P.value = as.numeric(Adjusted.P.value),
                                  Old.P.value = as.numeric(Old.P.value),
                                  Old.Adjusted.P.value = as.numeric(Old.Adjusted.P.value),
...
plot_data_all <- 
  ...
  select(merged_cluster, database, GO, P = Adjusted.P.value) 
...
UMAP_treemaps_BP <- 
  pblapply(1:max(as.numeric(pig_gene_umap$merged_cluster)),
           function(i) {
             ...
             plot_data <- 
               plot_data_all %>% 
               filter(merged_cluster == i & database == "GO_Biological_Process_2018")
...
UMAP_treemaps_CC <- 
  pblapply(1:max(as.numeric(pig_gene_umap$merged_cluster)),
           function(i) {
             ...
             plot_data <- 
               plot_data_all %>% 
               filter(merged_cluster == i & database == "GO_Cellular_Component_2018")
...
UMAP_treemaps_MF <- 
  pblapply(1:max(as.numeric(pig_gene_umap$merged_cluster)),
           function(i) {
             ...
             plot_data <- 
               plot_data_all %>% 
               filter(merged_cluster == i & database == "GO_Molecular_Function_2018")
...
pdf(savepath("UMAP cluster GSEA treemaps BP.pdf"), width = 8, height = 5)
UMAP_treemaps_BP
dev.off()
pdf(savepath("UMAP cluster GSEA treemaps CC.pdf"), width = 8, height = 5)
UMAP_treemaps_CC
dev.off()
pdf(savepath("UMAP cluster GSEA treemaps MF.pdf"), width = 8, height = 5)
UMAP_treemaps_MF
dev.off()

pdf(savepath("UMAP cluster GSEA treemaps combined.pdf"), width = 20, height = 5)
lapply(1:length(UMAP_treemaps_BP),
       function(i) {
         UMAP_treemaps_BP[[i]] +
           ggtitle(i,
                   "Biological process") +
...
pdf(savepath("UMAP cluster scatter hex.pdf"), width = 8, height = 8)
plots
dev.off()
```
...
plot_data_genes %>%
  ...
  mutate(Gene = ifelse(Gene == "NULL", "", Gene)) %>%
  
  write_csv(savepath("UMAP cluster genes.csv"))

plot_data_GO <- 
  pig_gene_UMAP_GO_res %>% 
  mutate(GO = str_extract(Term, "GO\\:\\d*")) %>%
  select(merged_cluster, Term, GO, database, Overlap,
         `P-value` = `P.value`,
         `Adjusted P-value` = Adjusted.P.value, 
         Genes) 
  # filter(`Adjusted P-value` < 0.05) %>%
  # mutate(`Adjusted P-value` = format(`Adjusted P-value`, digits=2, nsmall=2))

plot_data_GO %>%
  write_csv(savepath("UMAP GO results.csv"))
...
plot_data_hyper <- 
  ...
  arrange(merged_cluster, `Adjusted P-value`)

plot_data_hyper %>%
  write_csv(savepath("UMAP hyper results.csv"))



plot_data_cluster_info <- 
  UMAP_annotations %>% 
  ...
  left_join(plot_data %>% 
              group_by(merged_cluster) %>% 
              count()) %>%
  left_join(pig_gene_cluster_enrich_hyper %>%
              filter(adj_pval < 0.001) %>% 
              group_by(merged_cluster) %>% 
              summarise(tissues = paste(sort(enhanced_tissues), collapse = ", ")))

plot_data_cluster_info %>%
  write_csv(savepath("UMAP cluster info.csv"))
...
UMAP_cluster_anno <- 
  readxl::read_excel("data/processed/UMAP cluster annotation 2.xlsx")
...
plot_data_hulls %>% 
  select(1:3) %>%
  write_csv(savepath("UMAP cluster outlines.csv"))
...
umap_pal %>%
  enframe("merged_cluster", "color") %>%
  write_csv(savepath("UMAP cluster colors.csv"))
####
...
plot_data_genes <- 
  ...
  select(merged_cluster,
         ID = enssscg_id, 
         Gene = display_name, 
         Specificity = specificity_category, 
         Distribution = distribution_category, 
         `Elevated in` = enhanced_tissues) 



plot_data_hulls_cluster <- 
  plot_data_hulls %>%
  ungroup() %>%
  left_join(UMAP_cluster_anno %>% 
              select(merged_cluster = 2, 
                     cluster_name = `cluster name?`) %>%
              mutate(merged_cluster = as.factor(merged_cluster)))
...
plot <- 
  ...
  ggplot(aes(V1, V2, text = paste0("cluster ", merged_cluster, "\n", cluster_name))) +
  # geom_hex(aes(fill = 1),
  #          data = plot_data,
  #          fill = "gray70",
  #          bins = plot_bins) +
  ...
  geom_polygon(data = plot_data_hulls_cluster,
               aes(X, Y, 
                   color = merged_cluster,
                   fill = merged_cluster),
  ...
  geom_text(data = . %>%
              group_by(merged_cluster) %>%
              summarise(mean_V1 = mean(X), mean_V2 = mean(Y)),
            aes(mean_V1, mean_V2, label = merged_cluster),
            size = 2,
            alpha = 0.6,
            show.legend = F, 
...
ggsave(savepath("UMAP applet plot.svg"), plot, width = 5, height = 4)
ggsave(savepath("UMAP applet plot.pdf"), plot, width = 5, height = 4)
...
ggsave(savepath("UMAP applet plot exp color.svg"), plot, width = 5, height = 4)
ggsave(savepath("UMAP applet plot exp color.pdf"), plot, width = 5, height = 4)
####

ui <- fluidPage(

  # App title ----
  titlePanel("Pig gene expression UMAP"),
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      textOutput("cluster"),
      textOutput("cluster_name"),
      textOutput("n_genes"),
      textOutput("enriched_tissues"),
      textOutput("click_pos")
    ...
    mainPanel(
      ...
      # Output: Histogram ----
      plotOutput("UMAP", click = "plot_click"),
...
server <- function(input, output) {
   ...
   plot_par <- reactiveValues(clicked_cluster = 33)
  ...
  observe({ 
    ...
    plot_par$clicked_cluster <- 
      
      plot_data_hulls_cluster %>%
      group_by(merged_cluster) %>%
      summarise(in_clust = sp::point.in.polygon(input$plot_click$x, 
                                                input$plot_click$y, 
                                                pol.x = X,
                                                pol.y = Y)) %>%
      ungroup() %>% 
  ...
  output$UMAP <- 
    renderPlot({
      ...
      plot + 
        geom_polygon(data = plot_data_hulls_cluster %>%
                       filter(merged_cluster == plot_par$clicked_cluster),
                     aes(X, Y),
                     color = "red",
                     fill = NA,
                     show.legend = F) 
  ...
  pblapply(1:max(as.numeric(pig_gene_umap$merged_cluster)),
           function(i) {
             p1 <- 
               ...
               ggtitle(paste0(i,
                              ": ",
                              pig_gene_umap %>% filter(merged_cluster == i) %>% nrow(),
                              " genes"),
                       "Top GO-terms") +
               stripped_theme 
...
# ((p + p) / p + 
#     plot_layout(heights = c(1, 2))) | (p /p / p) 
#   plot_layout(ncol = 2)
...
if(file.exists("data/processed/gene_overlap_comparison.Rdata")) {
  ...
  gene_overlap_data <- 
    ...
    left_join(pig_gene_comparison_classification %>%
              ...
              by = c("enssscg_id"), 
              suffix = c("_human", "_pig")) %>%
    mutate(enriched_tissues_pig = ifelse(enriched_tissues_pig == "",
                                         NA,
                                         enriched_tissues_pig),
           enriched_tissues_human = ifelse(enriched_tissues_human == "",
                                           NA,
                                           enriched_tissues_human)) %>%
    
    group_by(comparison_id, enriched_tissues_pig, enriched_tissues_human, 
             enssscg_id, ensg_id, 
             spec_score_human, spec_score_pig, 
             spec_category_human = specificity_category_human, 
             spec_category_pig = specificity_category_pig) %>%
    ...
    filter(!(is.na(enriched_tissues_human) & 
               is.na(enriched_tissues_pig) &
               !(spec_category_human %in% c("Low tissue specificity",
                                            "Not detected") &
                   spec_category_pig %in% c("Low tissue specificity",
                                            "Not detected")))) %>%
    ungroup() %>%
  ...
  save(list = c("human_pig_gene_class_comparison_long_all_orth",
                "human_pig_gene_class_comparison_long",
                "human_pig_gene_class_comparison", 
                "gene_overlap_data",
                "gene_overlap_data_summarized",
  ...
  human_pig_gene_class_comparison %>% 
    ...
    write_csv("data/processed/Human Pig classification overlap.csv")
...
ggsave(savepath("Overlap enrichment per tissue 2.pdf"), 
       height = 4, width = 3, useDingbats = F)
...
overlap_hyper_all %>% 
  write_csv("data/processed/pig_human_class_hyper.csv")
...
plot <- 
  ...
  facet_wrap(~ comparison_tissue, ncol = 4) +
...
ggsave(savepath("Figure4 scatter overlap.pdf"), plot = plot, width = 10, height = 10)
...
plot <- 
  ...
  facet_wrap(~ comparison_tissue, nrow = 3, strip.position = "top") +
...
ggsave(savepath("Figure4 scatter overlap 2.pdf"), plot = plot, width = 9, height = 10)
...
plot_genes <- 
  c("PLN", "CYP19A1", "CDH17", "SATB2", "PDHB",
    "NPPA", "CYP11A1", "DCXR",
    "SCARB1",
    "PAX5", "CD163", "CD3E")
...
plot_data %>%  
  ...
  geom_text(data = . %>% 
              filter(display_name == "PLN" & comparison_tissue == "Heart muscle" |
                     display_name == "CYP19A1" & comparison_tissue == "Testis" |  
                       display_name == "CDH17" & comparison_tissue == "Intestine" |  
                     display_name == "SATB2" & comparison_tissue %in% c("Intestine", "Brain")),
            aes(label = comparison_tissue), 
            vjust = -0.5) +
  facet_wrap(~display_name + species, ncol = 1, strip.position = "left") + 
  scale_fill_manual(values = tissue_colors_palette_full) +
  scale_y_continuous(expand = expansion(c(0, 0.2)), position = "right") +
  stripped_theme_facet + 
  ylab("PTPM") +
  theme(axis.text = element_blank(),
        ...
        axis.line = element_blank(),
        aspect.ratio=1)
ggsave(savepath("Figure5 IHC example barplots.pdf"), width = 2, height = 18)
...
plot_data <-
  gene_orthologs %>%
  filter(human_gene_name %in% plot_genes) %>%
  select(enssscg_id, ensg_id, gene_name = human_gene_name) %>%
  left_join(pig_atlas_tissue %>%
              filter(tissue_name %in% c("lymph node", "thymus", 
                                        "spleen", "tonsil")) %>%
  ...
  gather(species, ptpm, pig, human) %>%
  
  mutate(gene_name = factor(gene_name, plot_genes),
         tissue_name = factor(tissue_name, 
                              c("spleen", "thymus", "tonsil", "lymph node")),
         species = factor(species, c("pig", "human"))) %>%
  group_by(gene_name, species) %>%
  mutate(ptpm = ptpm / max(ptpm)) 

plot_data %>%  
  ggplot(aes(tissue_name, ptpm, fill = tissue_name)) +
  geom_col(show.legend = F,
           color = NA,
           size = 0.2) +
  facet_wrap(~gene_name + species, ncol = 1, strip.position = "left") + 
  scale_fill_manual(values = set_names(colorRampPalette(c("black", "#A1A8AA", "white"))(9)[2:5],
                                       rev(c("spleen", "thymus", "tonsil", "lymph node")))) +
  scale_y_continuous(expand = expansion(c(0, 0.2)), position = "right") +
  stripped_theme_facet + 
  ylab("PTPM") +
  theme(axis.text = element_blank(),
        ...
        axis.line = element_blank(),
        aspect.ratio=1)
ggsave(savepath("Figure5S lymph IHC example barplots.pdf"), width = 2, height = 11)
...
# Nervous system diseases: RHO, RBP3, CHRNB2, SCN2A, ARX, HCN1,
# 
# Cardiovascular diseases: APOE, PCSK9, NPPA
# 
# Endocrine and metabolic diseases: CYP11A1
# 
# Digestive system diseases: KLK4

plot_genes %>% 
  select(gene_name, disease_system) %>%
  arrange(disease_system)

plot_genes <-
  disease_gene_classes %>% 
  # inner_join(readxl::read_excel("data/meta/Final disease genes for heatmap.xlsx"),
  #            by = c("gene_name" = "Gene")) %>% 
  filter(gene_name %in% 
           c("RHO", "RBP3", "CHRNB2", "SCN2A", "ARX", "HCN1",
             "APOE", "PCSK9", "NPPA",
             "CYP11A1",
             "KLK4",
             "DCXR", "BMP15")) %>%
  ...
  filter(!grepl("Congen|Other", disease_system) |
           (gene_name %in% c("PCSK9", "APOE") & 
              disease_system == "Congenital disorders of metabolism") |
           gene_name %in% c("DCXR", "BMP1")) %>%
  filter(!(gene_name %in% c("APOE") & 
              disease_system == "Nervous system diseases")) %>%
  # filter(!(gene_name == "NKX2-5" & disease_system != "Cardiovascular diseases")) %>%
  # filter(!(gene_name == "LMNA" & disease_system != "Cardiovascular diseases")) %>%
  # filter(!(gene_name == "SLC34A1" & disease_system != "Musculoskeletal diseases")) %>%
...
plot_data %>% 
  ungroup() %>%
  select(gene_name, species, disease_system, disease_system) %>% 
  distinct() %>%
  mutate(species = toupper(substr(species, 1, 1))) %>%
  mutate(gene_name = factor(gene_name, gene_order),
         disease_system = factor(disease_system, disease_system_order)) %>%
  ...
  facet_grid(disease_system ~ ., scales = "free", space = "free") +
  theme_void() +
  theme(plot.margin = unit(c(0,0,0,0), "mm"),
  ...
  facet_grid(disease_system ~ ., scales = "free", space = "free", labeller = label_wrap_gen()) +
  # facet_wrap(~disease_system, ncol = 1, strip.position = "right") +
  stripped_theme_facet +
...
ggsave(savepath("Fig7 selected genes heatmap 4 2.pdf"),
       width = 7, height = 3)
...
plot_data_frac <- 
  ...
  arrange(cor) %>% 
  mutate(comparison_tissue = factor(str_to_sentence(comparison_tissue), 
                                    str_to_sentence(unique(comparison_tissue))),
         gene_name = factor(gene_name, paste0("TLR", 1:10)),
         species = factor(species, c("human", "pig"))) 
...
plot_data_frac %>%
  filter(!comparison_tissue %in% c("Retina", "Bone marrow")) %>%
  
  {ggplot(., aes(as.numeric(species), frac, fill = gene_name, label = label)) +
      geom_col(color = "white",
               size = 0.3,
               width = 0.75) +
      geom_area(aes(scales::rescale(as.numeric(species), c(1.375, 1.625))),
                size = 0.3,
                color = "white",
                alpha = 0.4) +
      geom_text(size = 3,
      ...
      theme(panel.border = element_blank(),
            ...
            legend.position = "top")}
ggsave(savepath("TLR species fraction bar 2.pdf"), width = 7, height = 7)

plot_data_frac %>%
  filter(!comparison_tissue %in% c("Retina", "Bone marrow")) %>%
  filter(comparison_tissue %in% c("Heart muscle", 
                                  "Liver", 
                                  "Kidney",
                                  "Lung")) %>%
  {ggplot(., aes(as.numeric(species), frac, fill = gene_name, label = label)) +
      ...
      geom_text(size = 3,
                position = position_stack(vjust = 0.5), color = "white") +
      facet_wrap(~comparison_tissue, nrow = 2) +
      scale_x_discrete(labels = levels(.$species),
                       limits=c("1","2"),
                       expand = expansion(0)) +
      scale_y_continuous(expand = expansion(c(0, 0))) +
      ...
      theme(panel.border = element_blank(),
            ...
            strip.background = element_rect(color = NA),
            legend.position = "top")}
ggsave(savepath("TLR species fraction bar examples.pdf"), width = 3, height = 4)
...
#       theme(panel.border = element_blank(),
#             axis.title.x = element_blank())}
# ggsave(savepath("TLR species fraction bar 3.pdf"), width = 10, height = 7)
# 
# plot_data_frac %>%
#   
#   
...
#       theme(panel.border = element_blank(),
#             axis.title.x = element_blank())}
# ggsave(savepath("TLR species fraction bar 4.pdf"), width = 10, height = 7)
# ####
...
# plot_data1 <- 
#   plot_data_frac %>% 
#   select(gene_name, comparison_tissue, species, frac) %>%
#   unite(id, comparison_tissue, species) %>% 
#   spread(id, frac) %>% 
#   filter(complete.cases(.)) %>%
...
# plot_data <- 
#   plot_data1 %>%
#   uwot::umap(n_neighbors = 15, n_epochs = 1000)
# 
# plot_data %>% 
#   as.tibble() %>% 
#   mutate(id = rownames(plot_data1)) %>% 
...
plot_data_frac %>%
  ...
  column_to_rownames("gene_name") %>% 
  pheatmap(clustering_method = "ward.D2",
           color = cartography::carto.pal("orange.pal", n1 = 20),
           border_color = NA,
           ...
           annotation_colors = list(Species = overlap_type_pal[c(1,3)], 
                                    Tissue = tissue_colors_palette_comparison),
...
selected_blood_markers <- 
  ...
  bind_rows(tibble(gene_name = c("CD19",
                                 "CD3D",
                                 "CD3E", 
                                 "CD3G",
                                 "LCK"),
...
plot_gene_list <- 
  left_join(human_gene_class_blood %>%
            select(1, 2, ts_score, enhanced_tissues),
          human_gene_class_bloodlineage %>%
            select(1, 2, ts_score, enhanced_tissues),
          by = "ensg_id",
          suffix = c("", "_lineage")) %>% 
  filter(specificity_category_lineage %in% c("Tissue enriched", "Group enriched")) %>%
  left_join(human_gene_mapping %>%
              select(ensg_id, gene_name)) %>% 
  select(1, gene_name, everything()) %>%
  mutate(enhanced_tissues_lineage = ifelse(enhanced_tissues_lineage == "granulocytes",
                                           enhanced_tissues,
                                           enhanced_tissues_lineage),
         enhanced_tissues_lineage = ifelse(enhanced_tissues_lineage == "neutrophil",
                                           "Neutrophils",
                                           enhanced_tissues_lineage)) %>%
  filter(!grepl(",", enhanced_tissues_lineage)) %>%
  filter(!is.na(enhanced_tissues_lineage)) %>%
  filter(!(enhanced_tissues_lineage == "monocytes" & 
             enhanced_tissues != "classical monocyte,intermediate monocyte,non-classical monocyte")) %>%
  
  arrange(-ts_score_lineage) %>%
  group_by(enhanced_tissues_lineage) %>%
  top_n(30, ts_score_lineage) #%>% 
...
plot_data <- 
  selected_blood_markers %>%
  bind_rows(plot_gene_list %>%
              select(gene_name, 
                     cell_type = enhanced_tissues_lineage, 
                     ensg_id)) %>%
  inner_join(gene_orthologs) %>%
  inner_join(plot_data1) %>%
  mutate(cell_type = gsub("-", " ", cell_type),
         cell_type = case_when(cell_type %in% c("monocytes", "dendritic cells") ~ str_to_sentence(cell_type),
                               T ~ cell_type)) %>%
...
plot_data %>%
  ggplot(aes(frac, tissue_name, fill = species, color = species)) +
  geom_boxplot() +
  geom_point() +
...
lapply(unique(plot_data$cell_type),
       function(cell_type_) {
         ...
         plot_data %>% 
           ...
           select(gene_name, tissue_name, species, frac) %>%
           distinct %>%
           spread(gene_name, frac) %>% 
           unite(id, tissue_name, species) %>% 
           column_to_rownames("id") %>%
           pheatmap(clustering_method = "ward.D2",
                    color = cartography::carto.pal("orange.pal", n1 = 20),
                    main = cell_type_,
                    filename = savepath(paste0(cell_type_, ".pdf")))
       })
dev.off()              
# 
# lapply(unique(plot_data$cell_type),
...
#            ungroup() %>%
#            filter(cell_type == cell_type_) %>% 
#            select(gene_name, tissue_name, species, ptpm) %>%
#            distinct %>%
#            spread(gene_name, ptpm) %>% 
#            unite(id, tissue_name, species) %>% 
#            column_to_rownames("id") %>%
#            {log10(. + 1)} %>%
#            pheatmap(clustering_method = "ward.D2",
#                     color = cartography::carto.pal("orange.pal", n1 = 20),
#                     main = cell_type_,
#                     filename = savepath(paste0(cell_type_, " ptpm.pdf")), 
#                     display_numbers = F)
#        })
# dev.off() 
plot_data %>% 
...
###Blood cell composition - selected genes
...
plot_data %>%
  ...
  mutate(frac = frac * 100,
         label = ifelse(frac >= 5, 
                        ...
                        NA),
         species = factor(species, c("human", "pig")),
         tissue_name = factor(tissue_name, c("lymph node", "tonsil", "thymus", "spleen"))) %>% 
...
plot_data <- 
  disease_gene_classes %>%
  filter(!grepl("Congen|Other|Cancer", disease_system)) %>%
  left_join(gene_orthologs %>% 
              select(1:2)) %>%
  left_join(gene_overlap_data_summarized) %>% 
...
g <- 
  ...
  mutate(stratum = factor(stratum, c(plot_order,
                                     ...
                                     "Overlap"))) %>%
  left_join(plot_data %>% 
              group_by(disease_system, stratum = type) %>% 
              count() %>%
              group_by(disease_system) %>%
              mutate(frac = n / sum(n)) %>%
              ungroup %>%
              arrange(frac) %>%
              mutate(disease_system = factor(disease_system, filter(., stratum == "Overlap") %$% 
                                               unique(disease_system)),
  ...
  mutate(disease_system = factor(disease_system, filter(., stratum == "Overlap") %$% 
         ...
         stratum = factor(stratum, 
                          unique(c("No ortholog", "No overlap", "Not elevated",
                                   rev(sort(unique(stratum))))))) %>%
...
g
ggsave(savepath("Disease system overlap tissue alluvials.pdf"), plot = g, width = 6, height = 5, 
       useDingbats = F)  
  ...
  mutate(disease_system = factor(disease_system, filter(., type == "Overlap") %$% 
                                   ...
                                   {ggplot(., aes(frac * 100, disease_system,
                                       ...
                                       theme(axis.text.y = element_blank(),
                                             axis.ticks.y = element_blank(),
                                             axis.title = element_blank())}
ggsave(savepath("Disease system overlap barplot sup.pdf"), width = 7, height = 4)  
...
g <- 
  ...
  theme(axis.text = element_blank(),
        ...
        strip.text.y = element_text(angle = 0, hjust = 0))
g
ggsave(savepath("Disease system overlap tissue alluvials sup.pdf"), plot = g, width = 6, height = 3,
       useDingbats = F)  
...
plot_data %>%
  ...
  geom_stratum(show.legend = F,
               color = NA) +
  geom_text(stat = "stratum", 
            aes(label = after_stat(paste0(as.character(stratum), "\n", ymax-ymin))),
            reverse = T, 
            fontface = "bold",
            color = "white",
            angle = 90,
...
alluv_1 <-
  ...
  plot_data %>%
   mutate(Pig = str_to_sentence(specificity_category_pig),
          Human = str_to_sentence(specificity_category_human)) %>%
  select(Pig,
         Human) %>%
  mutate(row_n = row_number()) %>%
  gather(bar, chunk, -row_n) %>%
  ...
  mutate(chunk = factor(chunk, levels = c('Tissue enriched', 'Group enriched', 
                                          'Tissue enhanced', 'Low tissue specificity', 
                                          'Not detected', "No ortholog")),
         bar = factor(bar, levels = c("Pig",
                                      "Human"))) %>%
  ...
  scale_x_discrete(expand = c(.1, .1), position = "top") +
  scale_fill_manual(values = c(gene_category_pal, elevation_identity_pal, "No ortholog" = "gray45")) + 
...
ggsave(savepath("Disease gene specificity alluvial.pdf"), plot = alluv_2, width = 7, height = 2.5, useDingbats = F)
...
ggsave(savepath("All no overlap disease genes.pdf"),
       width = 8, height = 110, limitsize = F)
...
lapply(session_pkg$otherPkgs,
       function(x) x$Version) %>%
  plyr::ldply(.id = "name") %>%
  as_tibble() %>% 
  arrange(as.character(name)) %>%
  mutate(label = paste0(name, " (v ", V1, ")")) %>%
  pull(label) %>% 
  paste(collapse = ", ") %>% 
  cat()

lapply(session_pkg$loadedOnly,
       function(x) x$Version) %>%
  plyr::ldply(.id = "name") %>%
  as_tibble() %>% 
  arrange(as.character(name)) %>%
...
names(session_pkg$loadedOnly) %>% 
  sort() %>%
  paste(collapse = "\n") %>%
  cat()

"ggplot2 (v 3.3.2), concaveman (v 1.1.0), dendextend (v 1.14.0), ellipse (v 0.4.2), ggalluvial (v 0.12.2), ggalt (v 0.4.0), ggbeeswarm (v 0.6.0), ggdendro (v 0.1.22), ggforce (v 0.3.2), ggraph (v 2.0.3), ggrepel (v 0.8.2), ggridges (v 0.5.2), ggthemes (v 4.2.0), igraph (v 1.2.6), patchwork (v 1.0.1), pcaMethods (v 1.80.0), pheatmap (v 1.0.12), rrvgo (v 1.0.1), sf (v 0.9-6), tidygraph (v 1.2.0), treemapify (v 2.5.3), umap (v 0.2.6.0), uwot (v 0.1.8), viridis (v 0.5.1)" %>%
  str_split(", ") %>%
  {.[[1]]} %>%
  gsub(" \\(v.*$", "", .) %>%
  lapply(function(x) {
    capture.output(utils:::print.bibentry(citation(x), style = "Bibtex"),
                   file = savepath(paste0(x, ".bib")))}) 


"Cytoscape (v 3.6.1), limma (v 3.44.3), fpc (v 2.2-8), enrichR (v 2.1), Affinity designer (v 1.8.5.703), Blender (v 2.90.1), ggplot2 (v 3.3.2), concaveman (v 1.1.0), dendextend (v 1.14.0), ellipse (v 0.4.2), ggalluvial (v 0.12.2), ggalt (v 0.4.0), ggbeeswarm (v 0.6.0), ggdendro (v 0.1.22), ggforce (v 0.3.2), ggraph (v 2.0.3), ggrepel (v 0.8.2), ggridges (v 0.5.2), ggthemes (v 4.2.0), igraph (v 1.2.6), patchwork (v 1.0.1), pcaMethods (v 1.80.0), pheatmap (v 1.0.12), rrvgo (v 1.0.1), sf (v 0.9-6), tidygraph (v 1.2.0), treemapify (v 2.5.3), umap (v 0.2.6.0), uwot (v 0.1.8), viridis (v 0.5.1)" %>%
  str_split(", ") %>%
  unlist() %>%
  enframe() %>% 
  separate(value, into = c("pkg", "v"), sep = " \\(v ") %>%
  mutate(v = gsub("\\)", "", v),
         id = paste(pkg, v)) %>%
  arrange(pkg) %>%
  write_csv(savepath("KRT packages.csv"))



capture.output(utils:::print.bibentry(RStudio.Version()$citation, style = "Bibtex"),
               file = savepath("RStudio.bib"))

capture.output(utils:::print.bibentry(citation(), style = "Bibtex"),
               file = savepath("R.bib"))


capture.output(utils:::print.bibentry(citation("fpc"), style = "Bibtex"),
               file = savepath("fpc endnote.bib"))

capture.output(utils:::print.bibentry(citation("enrichR"), style = "Bibtex"),
               file = savepath("enrichR endnote.bib"))

capture.output(utils:::print.bibentry(citation("stats"), style = "Bibtex"),
               file = savepath("stats endnote.bib"))

```

#Save tables 

##Save norm sample data
...
#                      tissue_name)) %>% 
#   select(enssscg_id, individual, tissue_name, tpm, ptpm, tmm) %>% 
#   write_csv(savepath("sample data.csv"))

RColorBrewer::brewer.pal(n = 3, name =
"RdYlBu")
plot_data <- 
  pig_atlas_sample_temp %>% 
  select(1, 2, 5) %>% 
  group_by(enssscg_id) %>% 
  mutate(log_exp = log10(tpm + 1),
         z_score = (log_exp - mean(log_exp)) / sd(log_exp)) %>%
  ungroup()
  
plot <- 
  plot_data %>% 
  filter(enssscg_id %in% sample(unique(enssscg_id), 1000)) %>%
  ggplot(aes(sample_ID, enssscg_id, fill = log_exp)) +
  geom_tile() +
  scale_fill_viridis()+
  # scale_fill_gradientn(colours = RColorBrewer::brewer.pal(n = 11, name = "RdYlBu")) +
  theme_void() +
  coord_fixed()
  
ggsave(savepath("Pig expression matrix.png"), plot = plot, 
       dpi = 300,
       width = 10, height = 30, limitsize = F)
# scale() %>%
...
# read_tsv("data/meta/Sequencing QC data/QC-tissue.tsv") %>%
#   separate(Sample, into = c("individual", "tissue"), sep = "-") %>%
#   left_join(tissue_mapping %>% 
#               select(tissue, tissue_name)) %>% 
#   mutate(individual = letters[as.numeric(individual)]) %>% 
#   filter(!complete.cases(.))
#   left_join(read_csv("scripts/QC/Removed samples.csv") %>% 
#               select(-1))
```
...
anova_meta <- 
  colnames(pig_atlas_sample_wide_tmm) %>% 
  enframe("i", "sample_id") %>% 
  separate(sample_id, into = c("tissue_id", "individual"),
           remove = F, sep = "_") %>% 
  left_join(sample_meta) %>% 
  left_join(tissue_mapping %>% 
              select(tissue_id = tissue, tissue_name))

all(colnames(anova_data) == anova_meta$sample_id)

anova_tissues <- 
  pig_atlas_sample_norm %>% 
  select(sample_ID, individual, tissue_ID) %>% 
  distinct() %>% 
  group_by(tissue_ID) %>% 
  count() %>% 
  ungroup() %>% 
...
anova_genes <- 
  pig_atlas_sample_norm %>% 
  filter(tissue_ID %in% anova_tissues$tissue_ID) %>% 
  group_by(enssscg_id) %>% 
  filter(any(tmm >= 1)) %>% 
  ungroup() %>% 
  select(enssscg_id) %>% 
...
pig_atlas_sample_norm %>% 
  filter(tissue_ID %in% anova_tissues$tissue_ID) %>% 
  filter(enssscg_id %in% anova_genes$enssscg_id) %>% 
  # filter(enssscg_id %in% sample(unique(enssscg_id), 10)) %>% 
  select(enssscg_id, individual, tissue_ID, tmm) %>% 
  left_join(sample_meta) %>% 
  left_join(tissue_mapping %>% 
              select(tissue_ID = tissue, tissue_name)) %>% 
  select(-tissue_ID) %>% 
...
design <-
  model.matrix(~ 0 + sex + tissue_name,# + tissue_name:sex + sex, 
               anova_meta %>% 
                 filter(tissue_id %in% anova_tissues$tissue_ID)) 
...
fit <- limma::lmFit(anova_data[anova_genes$enssscg_id,
                               which(gsub("_.$", "", 
                                          colnames(anova_data)) %in% 
                                       anova_tissues$tissue_ID)],
                    design)

fit_contrasts <- makeContrasts(sex = sexmale - sexfemale,
                              
                           levels=design)

fit2 <- limma::contrasts.fit(fit, fit_contrasts)
fit3 <- limma::eBayes(fit2, trend = FALSE)
lapply(colnames(fit3$contrasts),
       FUN = function(con) limma::topTable(fit3, coef=con, number=Inf,
  ...
  left_join(gene_chromosome %>% 
              select(variable = 1, 
                     chromosome = 4) %>% 
              distinct()) %>%   
  mutate(chromosome = ifelse(!chromosome %in% c("X", "Y"),#str_length(chromosome) > 2, 
                             "other", 
                             chromosome),
         color = abs(logFC) > 1) %>% 
  left_join(gene_mapping,
            by = c("variable" = "enssscg_id")) %>% 
  mutate(gene_label = ifelse(display_name == "NULL" | !color,
                             NA, 
                             display_name)) %>%
  # View
  ggplot(aes(logFC, -log10(adj.P.Val), color = color, label = gene_label)) +
  geom_vline(xintercept = c(-1, 1) * 1,
             linetype = "dashed") +
  geom_point(size =1) +
  geom_text(size = 2) +
  ...
  scale_color_manual(values = c("TRUE" = "black", "FALSE" = "gray")) +
  theme_bw()
ggsave(savepath("limma sex genes.pdf"),
       width = 10, height = 4)
##################################################
...
cluster_library(worker_cluster, c("dplyr",
                                  "tidyverse"))
cluster_copy(worker_cluster, c("lm",
                               "aov", 
                               "omega_sq"))
...
sample_anova <- 
  ...
  do({
    ...
    lm_model <- lm(tmm ~ sex + tissue_name, 
                   data = g_data)
    
    aov_model <- aov(lm_model)
...
rm(worker_cluster)
var_pal <- 
  c(Residuals = "darkgray",
    # individual = "#611052",
    sex = "#3E71A8",
    # "sex:tissue_name" = "#3E71A8",
    tissue_name = "orangered")


sample_anova %>% 
  group_by(term) %>% 
  summarise(omega_sq = sum(omega_sq)) %>% 
  ungroup() %>% 
  mutate(omega_sq = omega_sq / sum(omega_sq),
         term = factor(term, names(var_pal))) %>% 
  ggplot(aes(1, omega_sq, fill = term, 
             label = paste(round(100 * omega_sq, 1), 
                           "%"))) +
  geom_col() +
  geom_text(position = position_stack(vjust = 0.5)) +
  ...
  scale_y_continuous(breaks = 0:1, 
                     expand = expansion(0)) +
  theme_bw() +
  labs(y = "% Variance (omegaSq)") + 
  theme(axis.text.x = element_blank(),
        axis.ticks.x = element_blank(),
        axis.title.x = element_blank(),
        axis.line.y = element_line(),
        panel.border = element_blank())
ggsave(savepath("tissue anova omega bar.pdf"),
       width = 4, height = 3)

sample_anova %>% 
  mutate(term = factor(term, names(var_pal))) %>% 
  ggplot(aes(term, omega_sq, fill = term)) +
  geom_violin(scale = "width", draw_quantiles = 0.5) +
  scale_fill_manual(values = var_pal) +
  scale_y_continuous(breaks = 0:1) +
  theme_bw()+
  labs(y = "% Variance (omegaSq)") 
ggsave(savepath("tissue anova omega violin.pdf"),
       width = 6, height = 3)

plot_ann <- 
  sample_anova %>% 
  group_by(enssscg_id) %>% 
  filter(term %in% c("sex", "individual") & omega_sq > 0.25) %>% 
  ungroup() %>% 
  left_join(gene_mapping) %>%
  mutate(display_name = ifelse(display_name != "NULL",
                               display_name, 
                               NA))
...
sample_anova %>% 
  ...
  ggplot(aes(term, omega_sq, fill = term)) +
  geom_violin(scale = "width", draw_quantiles = 0.5) +
  geom_point(data = plot_ann) +
  geom_text_repel(data = plot_ann, 
  ...
  theme_bw()+
  labs(y = "% Variance (omegaSq)") 
ggsave(savepath("tissue anova omega violin labeled.pdf"),
       width = 6, height = 3)
...
plot <- 
  ...
  mutate(term = factor(term, names(var_pal)),
         enssscg_id = factor(enssscg_id,
                             filter(., term == "Residuals") %>% 
                               arrange(omega_sq) %>% 
                               pull(enssscg_id))) %>% 
  ggplot(aes(enssscg_id, omega_sq, fill = term)) +
  geom_col() +
  scale_fill_manual(values = var_pal) +
  scale_y_continuous(breaks = 0:1,
                     expand = expansion(0)) +
  ...
  theme(axis.text.x = element_blank(),
        axis.ticks.x = element_blank())
ggsave(savepath("tissue anova omega genecol.pdf"),
       plot = plot,
       width = 10, height = 5)
...
sample_anova %>% 
  filter(term == "sex") %>% 
  arrange(-omega_sq) %>% 
  filter(omega_sq > 0.25) %>% 
  left_join(gene_chromosome %>% 
              select(enssscg_id = 1, chromosome = 4) %>% 
              distinct) %>% 
  mutate(chromosome = factor(chromosome, c(1:23, "X", "Y", "MT"))) %>% 
  group_by(chromosome) %>% 
  count() %>% 
  ggplot(aes(chromosome, n, label = n)) +
  geom_col() +
  geom_text(vjust = 0) +
  theme_minimal()
ggsave(savepath("tissue anova sex gene chromosomes.pdf"),
       width = 4, height = 3)  


hclust_factor_labels <- 
  function(dat, x, y, value, 
           dist_method = "euclidian",
           clust_method = "ward.D2") {
    dat %>% 
      select(all_of(c(x, y, value))) %>% 
      rename(x = 1, 
             y = 2, 
             value = 3) %>% 
      spread(y, value) %>% 
...
plot_data <- 
  sample_anova %>% 
  group_by(enssscg_id) %>% 
  filter(any(term %in% c("sex", "individual") & omega_sq > 0.25)) %>% 
  ungroup() %>% 
  left_join(gene_chromosome %>% 
              select(enssscg_id = 1, chromosome = 4) %>% 
              distinct) %>% 
  left_join(gene_mapping) %>% 
  mutate(gene_label = ifelse(display_name == "NULL",
                             enssscg_id, 
                             display_name),
         chromosome = factor(ifelse(str_length(chromosome)>2, 
                                    "other", 
                                    chromosome), c(1:23, "X", "Y", "MT", "other")),
         term = factor(term, names(var_pal)),
         enssscg_id = factor(enssscg_id,
                             filter(., term == "Residuals") %>% 
                               arrange(omega_sq) %>% 
...
p1 <- 
  plot_data %>% 
  select(gene_label, chromosome) %>% 
  distinct() %>% 
  ggplot(aes(gene_label, chromosome)) +
  geom_point() +
  theme_bw() +
  theme(axis.text.x = element_blank(),
        axis.ticks.x = element_blank(),
...
p2 <- 
  plot_data %>% 
  ggplot(aes(gene_label, omega_sq, fill = term)) +
  geom_col() +
  scale_fill_manual(values = var_pal) +
  scale_y_continuous(breaks = 0:1,
                     expand = expansion(0)) +
...
p1/p2
ggsave(savepath("Highvar sex ind genes.pdf"),
       width = 8, height = 6)

# sample_anova %>%
#   rename(term1 = term,
#          omega_sq1 = omega_sq) %>% 
#   expand_grid(term2 = unique(.$term1)) %>% 
#   left_join(select(., enssscg_id, omega_sq2 = omega_sq1, term2 = term1) %>% 
#               distinct()) %>% 
#   # filter(term1 < term2) %>% 
#   ggplot(aes(omega_sq1, omega_sq2)) +
#   geom_hex(aes(fill = stat(log10(count)))) +
#   facet_grid(term2 ~ term1)
#   spread()
...
plot_data <- 
  ...
  separate(sample1, into = c("tissue1", "ind1"), sep = "_", remove = F) %>% 
  separate(sample2, into = c("tissue2", "ind2"), sep = "_", remove = F) %>% 
  mutate(type = ifelse(tissue1 == tissue2,
                       "same tissue", 
                       "diff tissue")) %>% 
  filter(sample1 != sample2) %>% 
  left_join(tissue_mapping %>% 
              select(tissue, tissue_name, organ_name),
            by = c("tissue1" = "tissue")) %>% 
  left_join(tissue_mapping %>% 
              select(tissue, organ_name2 = organ_name),
            by = c("tissue2" = "tissue")) 
  
plot_data %>% 

  left_join(filter(., type == "diff tissue") %>% 
              group_by(tissue_name) %>% 
              summarise(sd = sd(cor), 
                        mean = mean(cor))) %>% 
  mutate(z_score = (cor - mean) / sd) %>% 
  filter(type == "same tissue") %>% 
  arrange(z_score) %>% 
  mutate(tissue_name = factor(tissue_name, unique(tissue_name))) %>% 
  ggplot(aes(z_score, tissue_name)) +
  geom_boxplot()
   

# p0 <-
#   plot_data %>%
#   mutate(organ_type = ifelse(organ_name == organ_name2,
#                              "same organ", 
#                              "diff organ")) %>% 
#   ggplot(aes(cor)) +
#   geom_histogram(bins = 50) +
#   scale_x_continuous(limits = c(0.6, 1)) +
#   facet_wrap(~ organ_type, ncol = 1, scales = "free_y", 
...
p1 <-
  plot_data %>%
  mutate(organ_type = case_when(tissue1 == tissue2 ~ "same tissue",
                                organ_name == organ_name2 ~ "same organ",
                                organ_name != organ_name2 ~ "diff organ")) %>%
  ggplot(aes(cor)) +
...
p2 <- 
  ...
  group_by(sample1) %>% 
  mutate(cor_perc = rank(cor)/length(cor)) %>% 
  ungroup() %>% 
  filter(type == "same tissue") %>% 
  group_by(tissue_name) %>% 
  mutate(min = min(cor),
...
ggsave(savepath("Sample correlation dist.pdf"),
       width = 5, height = 11)
...
plot_data %>% 
  filter(tissue1 == "peri",
         type == "same tissue") %>% 
  select(ind1, ind2, cor) %>% 
  spread(ind2, cor) %>% 
  column_to_rownames("ind1") %>% 
  pheatmap

plot_data %>% 
  # filter(tissue1 == "peri") %>% View
  filter(sample1 == "peri_b") %>% 
  # View
  ggplot(aes(cor)) +
  geom_histogram() +
...
# 
# plot_data_dist <- 
#   sample_pca$scores[, 1:10] %>% 
#   dist() %>% 
#   as.matrix() %>% 
#   as_tibble(rownames = "sample1") %>% 
#   gather(sample2, dist, -1) %>% 
#   separate(sample1, into = c("tissue1", "ind1"), sep = "_", remove = F) %>% 
#   separate(sample2, into = c("tissue2", "ind2"), sep = "_", remove = F) %>% 
#   mutate(type = ifelse(tissue1 == tissue2,
#                        "same tissue", 
...
exp_rank_data <- 
  pig_atlas_tissue %>% 
  select(enssscg_id, tissue_name, nx) %>% 
  # filter(enssscg_id %in% sample(enssscg_id, 100)) %>%
  group_by(enssscg_id) %>% 
  filter(max(nx) > 0) %>% 
  mutate(nx = nx / max(nx),
         nx_rank = rank(nx, ties.method = "random")) %>% 
  ungroup() %>% 
  select(-tissue_name)
...
exp_rank_clusters <- 
  cutree(exp_rank_clust, 10) %>% 
  enframe("enssscg_id", "spec_cluster")


exp_rank_score <- 
  exp_rank_data %>% 
  group_by(enssscg_id) %>% 
  summarise(score = sum(nx) / max(nx_rank),
            tau = sum(1 - nx) / (max(nx_rank) - 1))



example_gene <- 
  sample(exp_rank_data$enssscg_id, 1) 

exp_rank_data %>% 
  filter(enssscg_id == example_gene) %>% 
  mutate(nx_rank = nx_rank / max(nx_rank)) %>% 
  ggplot(aes(nx_rank, nx)) +
  geom_point() +
  geom_line() +
  geom_area(alpha = 0.5, 
            fill = "orangered") +
  theme_bw() +
  ggtitle(exp_rank_score %>% 
            filter(enssscg_id == example_gene) %>% 
            with(paste(enssscg_id, 
                       "\narea score =", round(score, 3), 
                       "\ntau score =", round(tau, 3))))
ggsave(savepath("Tau example single.pdf"),
       width = 5, height = 4)
exp_rank_score %>% 
  arrange(score) %>% 
  left_join(gene_mapping %>% 
  ...
  mutate(tau_class = case_when(tau > 0.9 ~ "high", 
                               tau > 0.65 ~ "medium", 
                               tau > 0.4 ~ "low",
                               T ~ "very low")) %>% 
  group_by(tau_class) %>% 
  slice(sample(1:length(tau), 3)) %>% 
  ungroup() %>% 
  inner_join(exp_rank_data) %>% 
  mutate(nx_rank = nx_rank / max(nx_rank),
         tau_class = factor(tau_class, 
                            c("high", 
                              "medium",
                              "low",
                              "very low"))) %>% 
  ...
  geom_area(alpha = 0.5, 
            fill = "orangered") +
  theme_bw() +
  facet_wrap(~ tau_class + 
               display_name + 
               specificity_category +
               enhanced_tissues, ncol = 6) 
ggsave(savepath("Tau examples.pdf"),
       width = 10, height = 6)
exp_rank_score %>% 
  ggplot(aes(score, tau)) +
  geom_hex(bins = 1000) +
  geom_abline(slope = -1, intercept = 1)

plot_order <- 
  exp_rank_score %>% 
  inner_join(exp_rank_clusters) %>% 
...
p0 <- 
  exp_rank_score %>% 
  inner_join(exp_rank_clusters) %>% 
  mutate(spec_cluster = factor(spec_cluster, plot_order)) %>% 
  ggplot(aes(spec_cluster, score)) +
  geom_violin() +
  theme_bw() +
  theme(axis.text.x = element_blank(),
        axis.title.x = element_blank(),
        axis.ticks.x = element_blank()) +
  facet_wrap(~ spec_cluster, nrow = 1, scales = "free_x") 

p1 <-
  pig_gene_classification %>% 
  inner_join(exp_rank_clusters) %>%
  select(enssscg_id, specificity_category, enhanced_tissues, spec_cluster) %>% 
    gather(type, value, enhanced_tissues, specificity_category) %>% 
    mutate(value = ifelse(is.na(value), 
                          "not enriched", 
                          value)) %>% 
    separate_rows(value, sep = ";") %>% 
  group_by(type, spec_cluster, value) %>% 
  count() %>% 
  ungroup() %>% 
  mutate(spec_cluster = factor(spec_cluster, plot_order))  %>% 
    left_join(tissue_mapping %>% 
                select(consensus_tissue_name, organ_name) %>% 
                distinct(), 
              c("value" = "consensus_tissue_name")) %>% 
    mutate(value = ifelse(is.na(organ_name), 
                          value, 
                          organ_name)) %>% 
  ggplot(aes(type, n, fill = value)) +
  geom_col(show.legend = F) +
  scale_fill_manual(values = c(gene_category_pal, tissue_colors_palette_organs)) +
  theme_bw() +
  theme(axis.text.x = element_blank(),
        axis.title.x = element_blank(),
        axis.ticks.x = element_blank()) +
...
p2 <- 
  ...
  inner_join(exp_rank_clusters) %>% 
  mutate(spec_cluster = factor(spec_cluster, plot_order)) %>% 
  # filter(ensssg_id %in% sample(enssscg_id, 100)) %>% 
  ggplot(aes(nx_rank, nx))+
  # geom_line(alpha = 0.1) +
  geom_hex(aes(fill = stat(log10(count))),
           bins = 100) +
  # geom_smooth() +
  facet_wrap(~ spec_cluster, nrow = 1) +
  theme_bw() 
  
p0 / p1 / p2  
ggsave(savepath("Tau cluster summary.pdf"),
       width = 12, height = 6)
...
exp_polys <-
  exp_rank_data %>%
  expand_grid(poly_degree = 15) %>% 
  group_by(poly_degree, enssscg_id) %>% 
  partition(worker_cluster) %>% 
  do({
    g_data <<- .
    poly_fit <- lm(nx ~ poly(nx_rank, unique(g_data$poly_degree)),
                   data = g_data)
    
    poly_fit_sum <- 
      summary(poly_fit)
    
    tibble(lm = list(poly_fit)) %>% 
      mutate(r2 = poly_fit_sum$r.squared,
             r2_adj = poly_fit_sum$adj.r.squared)
  }) %>% 
  ungroup() %>% 
  collect() 
...
exp_polys_coef <- 
  exp_polys %>% 
  group_by(enssscg_id, poly_degree, r2_adj) %>% 
  partition(worker_cluster) %>% 
  do({
    coefficients(.$lm[[1]]) %>% 
      enframe("coefficient", "value") 

  }) %>% 
  ungroup() %>% 
  collect() %>% 
  mutate(coefficient = paste0("c", str_extract(coefficient, "I|\\d$"))) 

exp_polys %>% 
  select(poly_degree, enssscg_id, r2, r2_adj) %>% 
  mutate(poly_degree = factor(poly_degree)) %>% 
  ggplot(aes(r2_adj, poly_degree)) +
  geom_violin(fill = "orangered") +
  theme_bw()

# sim_poly_data <-
#   exp_polys %>% 
#   # filter(poly_degree == 3) %>% 
#   # filter(r2_adj > 0.99) %>% 
#   group_by(poly_degree, enssscg_id) %>% 
#   partition(worker_cluster) %>% 
#   do({
#     g_data <<- .
#     
#     tibble(x = seq(0, 1, length.out = 50),
#            y = predict(g_data$lm[[1]], newdata = tibble(nx_rank = x)))
#   }) %>% 
#   ungroup() %>% 
#   collect() %>% 
#   inner_join(exp_rank_clusters) 
...
#   
# sim_poly_data %>% 
#   ggplot(aes(x, y, group = enssscg_id)) +
#   geom_line(alpha =0.3) +
#   theme_bw()
...
# predicted_df <- 
#   exp_polys %>% 
#   filter(enssscg_id == plot_gene) %>%
#   filter(poly_degree == 3) %>% 
#   pull(lm) %>% 
#   {.[[1]]} %>% 
#   {data.frame(y = predict(., 
#                          newdata = tibble(nx_rank = seq(0, 1,
#                                                         length.out = 50))))}
# 
#  predict(a, newdata = tibble(nx_rank = 1:10))
#   predict(newdata = tibble(nx_rank = x))
plot_gene <- sample(exp_polys$enssscg_id, 1)

exp_rank_data %>% 
  filter(enssscg_id == plot_gene) %>% 
  ggplot(aes(nx_rank, nx)) +
  geom_point() +
  stat_function(fun = function(x) {exp_polys %>% 
      filter(enssscg_id == plot_gene) %>%
      filter(poly_degree == 15) %>% 
      pull(lm) %>% 
      {.[[1]]} %>% 
      predict(newdata = tibble(nx_rank = x))})


# exp_polys_coef_clust <- 
  exp_polys_coef %>% 
  filter(poly_degree == 10) %>% 
  select(enssscg_id, coefficient, value) %>% 
  spread(coefficient, value) %>% 
  column_to_rownames("enssscg_id") %>% 
    pheatmap(clustering_method = "ward.D2", 
             cluster_cols = F, 
             show_rownames = F,
             annotation_row = exp_rank_clusters %>%
               mutate(spec_cluster = as.character(spec_cluster)) %>% 
               left_join(exp_polys %>% 
                           select(enssscg_id, r2_adj)) %>% 
               column_to_rownames("enssscg_id"))
# exp_rank_data %>% 
#       inner_join(exp_rank_clusters) %>% 
#       filter(spec_cluster == 13) %>% 
#   ggplot(aes(nx_rank, nx)) +
#   geom_hex() +
#   stat_function(fun=function(x) predict(gene_poly, newdata = tibble(nx_rank = x)))

exp_rank_data %>%
  inner_join(exp_rank_clusters) %>%
  mutate(spec_cluster = factor(spec_cluster, plot_order)) %>% 
  group_by(spec_cluster) %>% 
  filter(enssscg_id %in% sample(unique(enssscg_id), 1)) %>%  
  ungroup() %>% 
  arrange(spec_cluster) %>% 
  mutate(enssscg_id = factor(enssscg_id, unique(enssscg_id))) %>% 
  ggplot(aes(nx_rank, nx, group = enssscg_id)) +
  geom_point(size = 0.5, alpha = 0.5) +
  geom_line(alpha = 0.5) +
  facet_wrap(~spec_cluster) +
  theme_bw() +
  labs(y = "Relativt uttryckt till max-uttryck",
       x = "Rank")
...
exp_rank_data %>% 
  inner_join(exp_rank_clusters) %>% 
  mutate(spec_cluster = factor(spec_cluster, plot_order)) %>%
  arrange(spec_cluster, enssscg_id, nx_rank) %>% 
  group_by(enssscg_id) %>% 
  mutate(nx = nx / sum(nx),
         nx = cumsum(nx)) %>% 
  # filter(ensssg_id %in% sample(enssscg_id, 100)) %>% 
...
exp_rank_data %>% 
  ...
  mutate(spec_cluster = factor(spec_cluster, plot_order),
         enssscg_id = factor(enssscg_id,
                             with(exp_rank_clust, labels[order]))) %>% 
  ggplot(aes(enssscg_id, nx_rank, fill = nx)) +
...
exp_rank_score %>% 
  ...
  mutate(cum_n = row_number()) %>% 
  ungroup() %>% 
  ggplot(aes(score, cum_n, color = specificity_category)) +
  geom_line(size = 3) + 
...
exp_rank_score %>% 
  ...
  ggplot(aes(score, specificity_category, fill = specificity_category)) +
  geom_violin(show.legend = F, scale = "width", draw_quantiles = 0.5) +
  scale_fill_manual(values = gene_category_pal) +
  theme_bw()
ggsave(savepath("Tau distribution.pdf"),
       width = 5, height = 4)
...
tau_score <- 
  calculate_tau_score(wide_data)


exp_rank_score %>% 
  left_join(tau_score,
            by = c("enssscg_id" = "gene")) %>% 
  filter(tau != tau_score)
