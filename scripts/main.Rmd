...
plot <- 
  ...
  geom_hex(data = plot_data2 %>%
           ...
           bins = plot_bins,
           show.legend = F) +
  facet_wrap(~ enhanced_tissues, ncol = 6) +
  theme_void()

# plot
ggsave(savepath("Gene UMAP all tissues.pdf"), plot = plot, 
       width = plot_width * 6/4, 
       height = plot_height * 8/4)
...
plot_data_GO <- 
  ...
  select(merged_cluster, Term, database, Overlap,
         `P-value` = Adjusted.P.value, Genes) %>% 
  filter(`P-value` < 0.05) %>%
  mutate(`P-value` = format(`P-value`, digits=2, nsmall=2))
