...
#       # saved_data <<- g_data
#       
#       model <- aov(log_tmm ~ tissue_name,
#                    data = g_data)
#       
#       
#       
#       # model <- aov(log2(tmm + 1) ~ tissue_name + tissue_name:sex + sex,
#       #              data = g_data)
#       
#       aov_res <- 
#         model %>%
...
#   #####
#     
#   A %>% spread(zero, n, fill = 0) %>% mutate(ratio = `TRUE`/(`TRUE` + `FALSE`)) %>% 
#     filter(ratio != 0) %>%
#     ggplot(aes(ratio)) + 
#     geom_histogram() + 
...
intraconsens_tissue_cluster_dict <- 
  ...
  summarise(tissue_clusters = paste(unique(tissue_cluster), collapse = ","))
...
intraconsens_gene_enrich_overlap <- 
  ...
  left_join(pig_gene_classification %>% 
              separate_rows(enhanced_tissues, sep = ","),
            by = c("enssscg_id")) %>%
  group_by(consensus_tissue_name, enssscg_id, high_tissues, n_high, low_tissues, n_low) %>%
  summarise(overlap = case_when(any(is.na(enhanced_tissues)) ~ "not tissue elevated",
                                any(enhanced_tissues == consensus_tissue_name) ~ "yes",
                                T ~ "no")) %>%
  left_join(pig_gene_classification,
            by = c("enssscg_id")) %>%
  ungroup()
...
intraconsens_gene_enrich_overlap %>%
  ...
  group_by(consensus_tissue_name, high_tissues, overlap) %>% 
  summarise(n = n_distinct(enssscg_id)) %>% 
  
  group_by(high_tissues) %>%
  mutate(total_n = sum(n)) %>%
  ungroup() %>%
  arrange(total_n, high_tissues, n) %>% 
  mutate(high_tissues = factor(high_tissues, unique(high_tissues))) %>%
  ggplot(aes(n, high_tissues, fill = overlap)) +
  geom_col() +
  scale_fill_manual(values = c("no" = "orangered", "yes" = "darkblue", "not tissue elevated" = "gray")) +
  stripped_theme_facet +
  ...
  theme(strip.text.y = element_text(angle = 0),
        panel.spacing = unit(0, "mm"))
ggsave(savepath("Intraconsens bar n genes overlap.pdf"), width = 8, height = 8)  
...
intraconsens_heatmap_data %>% 
  select(consensus_tissue_name, enssscg_id) %>% 
  unique() %>% 
  pull(consensus_tissue_name) %>%
  table()
         ...
         function(orgn) {
           ...
           g <- 
             ...
             pheatmap(clustering_method = "ward.D2", 
                      ...
                      annotation_colors = list(overlap = c("no" = "orangered", 
                                                           "yes" = "darkblue", 
                                                           "not tissue elevated" = "gray",
                                                           "elevated in tissue" = "white")) ,
                      main = orgn,
                      cutree_rows = heatmap_settings$cutree_genes,
                      cutree_cols = heatmap_settings$cutree_tissue,
                      silent = T)
...
xeno_ko_genes$enssscg_id %in% as_tibble(net_graph)$name
