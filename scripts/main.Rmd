...
if(file.exists("data/processed/pig_intraconsensus_anova.Rdata")) {
  ...
  cluster_copy(cluster, names = c("%>%", "anova_data", "%$%", 
                                  "omega_sq" ) )
  ...
  intraconsens_anova_temp <- 
    ...
    do({
      ...
      aov_res <- 
        model %>%
        broom::tidy() %>%
        dplyr::left_join(omega_sq(model) %>% 
                           tibble::enframe("term", "sqrt_omega"), 
                         by = "term")
      
      
      tukey_res <- 
        TukeyHSD(model) %>%
        purrr::map(.f = function(x) tibble::as_tibble(x, rownames = "Comparison")) %>%
        dplyr::bind_rows(.id = "term") %>%
        tidyr::separate(Comparison, into = c("tissue1", "tissue2"), sep = "-") 
      
      
      aov_res %>% 
        dplyr::left_join(tukey_res,
                         by = "term") 
    }) %>%
    collect()
  ...
  intraconsens_anova_res <- 
    intraconsens_anova_temp %>% 
    left_join(intraconsens_anova_diffs) %>%
    select(consensus_tissue_name, enssscg_id, df, sumsq, meansq, sqrt_omega, statistic,
           aov_p = p.value, 
           tissue1, tissue2, 
           tukey_p = `p adj`,
           everything()) %>%
    ...
    mutate(sign_aov = adj_aov_p < 0.05,
           ...
           show = sign_aov & sign_contrast & relevant)
