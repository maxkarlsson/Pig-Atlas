...
if(file.exists("data/processed/pig_gene_spearman.Rdata")) {
  ...
  tot_combs <- 
    pig_gene_propr_res_05filtered %>%
    filter(enssscg_id1 %in% filtered_genes & enssscg_id2 %in% filtered_genes) %>% nrow
  n_gn <- 1000
  gene_combinations <- 
    pig_gene_propr_res_05filtered %>%
  ...
  # rcorr()
  # a <- lapply(1:nrow(gene_combinations),
  #             function(i) {
  #               cor.test(pig_gene_spearman_data[,gene_combinations$enssscg_id1[i]],
  #                        pig_gene_spearman_data[,gene_combinations$enssscg_id2[i]])%>%
  #       tidy()
  #             })
  
  tm <- Sys.time() - t1
  tm_per <- tm/nrow(gene_combinations)
  tm_tot<- tm_per * tot_combs
  
  tm_tot / 3600
  ((object.size(a)/nrow(gene_combinations)) * tot_combs)/10^9
  
  
  gene_combinations %>%
    left_join(pig_gene_spearman_data,
              c("V1" = "enssscg_id")) %>%
    left_join(pig_gene_spearman_data,
              c("V2" = "enssscg_id", "sample_ID"),
              suffix = c("1", "2")) 
