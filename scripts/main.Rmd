...
###Comparison spearman propr

```{r}

if(file.exists("data/processed/pig_gene_spearman_propr.Rdata")) {
  load("data/processed/pig_gene_spearman_propr.Rdata")
  
} else {
  temp_table_1 <-
    pig_gene_propr_res_05filtered %>% 
    filter(paste(enssscg_id1, enssscg_id2) %in% 
             paste(pig_gene_spearman_res$enssscg_id1, pig_gene_spearman_res$enssscg_id2)) %>%
    arrange(enssscg_id1, enssscg_id2)
  ...
  i_s <- 
    sample(1:nrow(temp_table_2), 10000)
  
  
  all(temp_table_1$enssscg_id1[i_s] == temp_table_2$enssscg_id1[i_s] )
  all(temp_table_1$enssscg_id2[i_s] == temp_table_2$enssscg_id2[i_s] )
  
  pig_gene_spearman_propr <-
    temp_table_1 %>%
    mutate(cor = temp_table_2$cor,
           cor_p = temp_table_2$p_val)
  
  rm(temp_table_1, temp_table_2)
  save(pig_gene_spearman_propr,
       file = "data/processed/pig_gene_spearman_propr.Rdata")
...
intraconsens_relevant_genes <- 
  intraconsens_anova_res %>%
  group_by(term) %>%
  mutate(adj_p2 = p.adjust(aov_p, method = "BH")) %>% 
  ungroup() %>%
  filter(sign_aov & sign_contrast & relevant & adj_p2 < 0.05) %>%
  left_join(pig_gene_classification) 
...
intraconsens_gene_enrich_overlap <- 
  ...
  left_join(intra_consensus_analysis_list %>%
              left_join(tissue_mapping %>% 
                          select(tissue_name, consensus_tissue_name),
                        by = c("tissues" = "tissue_name")) %>% 
              select(-tissues) %>% 
              distinct()) %>%
  mutate(overlap = case_when(is.na(enhanced_tissues) ~ "not tissue enriched",
                             enhanced_tissues == consensus_tissue_name ~ "yes",
                             enhanced_tissues != consensus_tissue_name ~ "no")) %>%
  group_by(organ, enssscg_id) %>% 
  summarise(is_overlap = case_when(any(overlap == "not tissue enriched") ~ "not tissue enriched",
                                   any(overlap == "yes") ~ "yes",
                                   T ~ "no"),
            overlapping_tissues = paste(sort(unique(consensus_tissue_name[which(overlap == "yes")])), collapse = ",")) %>%
  arrange(enssscg_id) %>% 
  ungroup()

intraconsens_final_res <- 
  intraconsens_relevant_genes %>%
  left_join(intraconsens_gene_enrich_overlap)

intraconsens_final_res %>%
  
  select(organ, enssscg_id, tissue1, tissue2, ratio, p_value = adj_p2, is_overlap, overlapping_tissues) %>%
  write_csv(savepath("intraconsensus results.csv"))

######

intraconsens_final_res %>%
  group_by(organ, is_overlap) %>% 
  summarise(n = n_distinct(enssscg_id)) %>% 
  
  group_by(organ) %>%
  mutate(total_n = sum(n)) %>%
  ungroup() %>%
  arrange(total_n, organ, n) %>% 
  mutate(organ = factor(organ, unique(organ))) %>%
  ggplot(aes(n, organ, fill = is_overlap)) +
  geom_col() +
  scale_fill_manual(values = c("no" = "orangered", "yes" = "darkblue", "not tissue enriched" = "gray")) +
  theme_minimal() +
  ggtitle("Intraconsensus classification", "Overlap with tissue enrichment classification")
ggsave(savepath("Intraconsens bar n genes overlap.pdf"), width = 6, height = 6)  
...
plots <- 
  lapply(intra_consensus_analysis_list %>% 
           select(organ) %>% 
           filter(!organ %in% c("Mesothelial tissue")) %>% 
           unique() %>% 
           pull(1),
         function(orgn) {
           print(orgn)
           ...
           g <- 
             ...
             pheatmap(clustering_method = "ward.D2", 
                      ...
                      annotation_colors = list(is_overlap = c("no" = "orangered", 
                                                              "yes" = "darkblue", 
                                                              "not tissue enriched" = "gray")) ,
                      main = orgn,
                      silent = T)
...
intraconsens_anova_res %>% 
  ...
  inner_join(intraconsens_final_res %>% 
               select(organ, enssscg_id) %>% 
               distinct()) %>%
  filter(organ == "Lymphoid tissues") %>% 
  filter(enssscg_id %in% enssscg_id[1:100]) %>%
  # group_by(enssscg_id) %>%
  select(organ, enssscg_id, tissue1, tissue2, ratio) %>%
  bind_rows(rename(., 
                   tissue1 = tissue2, 
                   tissue2 = tissue1) %>%  
              mutate(ratio = 1 / ratio)) %>%
  mutate(ratio = log2(ratio)) %>%
  spread(tissue2, ratio, fill = 0) %>%
  group_by(enssscg_id) %>%
  do({
    g_data <<- .
