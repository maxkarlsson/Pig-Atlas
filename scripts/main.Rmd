...
consensus_tissues_with_multiple <- 
  tissue_mapping %>% 
  filter(tissue %in% pig_atlas_tissue$tissue_ID) %>%
...
pig_gene_cluster_enrich_hyper %>% 
  ...
  spread(cluster, sign_level) %>% 
  column_to_rownames("enhanced_tissues") %>% 
  pheatmap(clustering_method = "ward.D2",
           color = colorRampPalette(c("white", "orange", "orangered"))(9),
           filename = savepath("Gene UMAP cluster elevated overlap sign level.pdf"),
           width = 16, 
           height = 6)
...
pig_gene_cluster_enrich_hyper %>% 
  ...
  pheatmap(clustering_method = "ward.D2",
           color = colorRampPalette(c("white", "orange", "orangered"))(9),
           filename = savepath("Gene UMAP cluster elevated overlap logp.pdf"),
           width = 16, 
           height = 6)
```
...
overlap_hyper_data <- 
  gene_overlap_data %>%
  mutate(comparison_id = ifelse(comparison_id == "", "not enriched", comparison_id)) %>%
  select(1:3, type) %>% 
...
overlap_hyper_data %>% 
  ...
  distinct() %$%
  # head%$% 
  tidyr::crossing(pig_id = comparison_id,
                  human_id = comparison_id) %>%
  inner_join(overlap_hyper_data,
             by = c("pig_id" = "comparison_id")) %>% 
  inner_join(overlap_hyper_data,
             by = c("human_id" = "comparison_id"))

overlap_hyper_test_data <-
  overlap_hyper_data %>% 
    ungroup() %>%
  select(comparison_id) %>% 
  distinct() %$%
  ...
  do({
    ...
    g_data <- 
      overlap_hyper_data %>% 
      summarise(overlap_success = any(type %in% c("Pig", "Overlap") & comparison_id == pig_id_) &
                  any(type %in% c("Human", "Overlap") & comparison_id == human_id_), 
                pig_elev = any(type %in% c("Pig", "Overlap") & comparison_id == pig_id_),
                human_elev = any(type %in% c("Human", "Overlap") & comparison_id == human_id_)) %>%
      ungroup()
      
    # q is the number of successes
    q <- 
      g_data$overlap_success %>% 
      which() %>% 
      length()
    
    # k is the number of tries - i.e. the number of genes that are elevated for either species
    k <- 
      (g_data$pig_elev | g_data$human_elev) %>% 
      which() %>% 
      length()
    
    # m is the number of possible successes - i.e. the number of genes that are elevated for either
    m <- 
      min(c(g_data$human_elev %>% 
              which() %>% 
              length(),
            g_data$pig_elev %>% 
              which() %>% 
...
overlap_hyper <- 
  overlap_hyper_test_data %>%
  group_by(pig_id, human_id, q, k, .drop = F) %>%
...
overlap_hyper %>% 
  group_by(pig_id, human_id) %>%
  mutate(capped_p = min(c(-log10(adj_pval), 50))) %>%
  ungroup() %>% 
...
overlap_hyper %>%
  select(human_id, pig_id, adj_pval) %>%
  
  spread(human_id, adj_pval, fill = 1) %>% 
  column_to_rownames("pig_id") %>%
  {-log10(.)} %>%
  pheatmap(clustering_method = "ward.D2",
           color = heatmap_palette)
...
overlap_enrich_hyper_data <- 
  gene_overlap_data %>%
  mutate(comparison_id = ifelse(comparison_id == "" |
                                  !(spec_category_pig %in% c("Tissue enriched", "Group enriched") |
                                      spec_category_human %in% c("Tissue enriched", "Group enriched")), "not enriched", comparison_id)) %>%
  select(1:3, type) %>% 
  group_by(enssscg_id, ensg_id)

overlap_enrich_hyper_test_data <-
  overlap_enrich_hyper_data %>% 
    ungroup() %>%
  select(comparison_id) %>% 
  distinct() %$%
...
overlap_enrich_hyper_data2 <- 
  gene_overlap_data %>%
  mutate(comparison_id = ifelse(comparison_id == "" |
                                  !(spec_category_pig %in% c("Tissue enriched") |
                                      spec_category_human %in% c("Tissue enriched")), "not enriched", comparison_id)) %>%
  select(1:3, type) %>% 
  group_by(enssscg_id, ensg_id)

overlap_enrich_hyper_test_data2 <-
  overlap_enrich_hyper_data2 %>% 
    ungroup() %>%
  select(comparison_id) %>% 
  distinct() %$%
...
# Update this plot with ungrouped tissues:
pig_atlas_tissue %>%
  filter(nx >= 1) %>%
  select(1:2, nx) %>% 
  left_join(tissue_mapping %>% 
  ...
  distinct() %>% 
  left_join(pig_gene_classification) %>%
  group_by(consensus_tissue_name, distribution_category) %>%
  ...
  mutate(distribution_category = str_to_sentence(distribution_category),
         ...
         consensus_tissue_name = str_to_sentence(consensus_tissue_name),
         consensus_tissue_name = factor(consensus_tissue_name, unique(consensus_tissue_name))) %>%
  ggplot(aes(n_detected, consensus_tissue_name, fill = distribution_category)) +
  geom_col(width = 0.8) +
  geom_text(data = . %>% 
              select(consensus_tissue_name, sum_n) %>% 
              distinct(), 
            aes(sum_n, consensus_tissue_name, label = sum_n), 
            hjust = 0,
            size = 3,
            inherit.aes = F) + 
...
ggsave(savepath("Supplementary detected genes bar.pdf"), width = 5, height = 7)  
