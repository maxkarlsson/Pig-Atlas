...
plot_data <- 
  plot_data1 %>%
  umap(n_neighbors = 5, n_epochs = 1000)
...
plot_data <- 
  joined_atlas_comparison %>% 
  separate(mutual_id, into = c("enssscg_id", "ensg_id")) %>% 
  inner_join(disease_genes_selected) %>%
...
plot_data3 <-
  ...
  left_join(disease_genes_selected)
...
plot_data %>% 
  ungroup() %>%
  select(gene_name, species, disease) %>% 
  distinct() %>%
...
ggsave(savepath("Fig7 selected genes heatmap 2.pdf"),
       width = 9, height = 8)
...
disease_gene_cor <-
  cor_data %>% 
  group_by(disease, enssscg_id, ensg_id, pig_gene_name, human_gene_name, ortholog_type, ortholog_confidence) %>% 
  do({
...
plot_data %>%
  ...
  mutate(frac = ptpm / sum(ptpm)) %>%
  ggplot(aes(species, frac, fill = gene_name)) +
  geom_col(color = "white") +
  facet_wrap(~comparison_tissue) +
  stripped_theme_facet
...
human_gene_class_blood %>%
  ...
  mutate(frac = mutual_tmm / sum(mutual_tmm)) %>%
